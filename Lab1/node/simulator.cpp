...
private:
    ...
    std::string map_frame, base_frame, base_frame2, scan_frame;
    ...
    std::map<double, CarState> last_pose;
...
public:
    ...
    RacecarSimulator(): im_server("racecar_sim") {
        ...
        n.getParam("base_frame2", base_frame2);
    ...
    void update_pose(const ros::TimerEvent&) {
        ...
        last_pose[current_seconds] = state;
        double max_time = 0.0;
        for (auto &a : last_pose) {
            if (a.first < current_seconds - 1.0) {
                max_time = std::max(max_time, a.first);
            }
        }
        if (max_time > 1.0) {
            state2 = last_pose[max_time];
            for (std::map<double, CarState>::iterator a = last_pose.begin(); a != last_pose.end();) {
                if (a->first < max_time) {
                    a = last_pose.erase(a);
//                    ROS_INFO("1");
                }
...
//        first_time = false;
        ...
        if (map_exists) {
            ...
            if (state.velocity != 0) {
                for (size_t i = 0; i < scan_.size(); i++) {
                    ...
                    double dtc = scan_[i] - car_distances[i];
                    double ttc = dtc / proj_velocity;
    ...
    void pub_pose_transform2(ros::Time timestamp) {
        // Convert the pose into a transformation
        geometry_msgs::Transform t2;
        t2.translation.x = state2.x;
        t2.translation.y = state2.y;
        tf2::Quaternion quat2;
        quat2.setEuler(0., 0., state2.theta);
        t2.rotation.x = quat2.x();
        t2.rotation.y = quat2.y();
        t2.rotation.z = quat2.z();
        t2.rotation.w = quat2.w();

        // publish ground truth pose
        geometry_msgs::PoseStamped ps2;
        ps2.header.frame_id = "/map";
        ps2.pose.position.x = state2.x;
        ps2.pose.position.y = state2.y;
        ps2.pose.orientation.x = quat2.x();
        ps2.pose.orientation.y = quat2.y();
        ps2.pose.orientation.z = quat2.z();
        ps2.pose.orientation.w = quat2.w();

        // Add a header to the transformation
        geometry_msgs::TransformStamped ts2;
        ts2.transform = t2;
        ts2.header.stamp = timestamp;
        ts2.header.frame_id = map_frame;
        ts2.child_frame_id = base_frame2;
    ...
    void pub_steer_ang_transform2(ros::Time timestamp) {
        // Set the steering angle to make the wheels move
        // Publish the steering angle
        tf2::Quaternion quat_wheel;
        quat_wheel.setEuler(0., 0., state2.steer_angle);
        geometry_msgs::TransformStamped ts_wheel;
        ts_wheel.transform.rotation.x = quat_wheel.x();
        ts_wheel.transform.rotation.y = quat_wheel.y();
        ts_wheel.transform.rotation.z = quat_wheel.z();
        ts_wheel.transform.rotation.w = quat_wheel.w();
        ts_wheel.header.stamp = timestamp;
        ts_wheel.header.frame_id = "front_left_hinge2";
        ts_wheel.child_frame_id = "front_left_wheel2";
        br.sendTransform(ts_wheel);
        ts_wheel.header.frame_id = "front_right_hinge2";
        ts_wheel.child_frame_id = "front_right_wheel2";
        br.sendTransform(ts_wheel);
    }
